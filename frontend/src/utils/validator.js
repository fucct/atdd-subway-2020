const validator = {
  path: {
    source: [(v) => !!v || '출발역 입력이 필요합니다.', (v) => v > 0 || '출발역 ID는 양수만 가능합니다.'],
    target: [(v) => !!v || '도착역 입력이 필요합니다.', (v) => v > 0 || '도착역 ID는 양수만 가능합니다.']
  },
  departureTime: {
    dayTime: [(v) => (v === 'am' || v === 'pm') || '오전 또는 오후만 입력 가능합니다.'],
    hour: [(v) => (v>=1 && v<=12) || '1시부터 12시 까지만 입력 가능합니다.'],
    minute: [(v) => (v>=0 && v<=59) || '0분 부터 59분 까지만 입력 가능합니다.']
  },
