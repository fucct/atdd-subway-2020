    ...
    <div class="d-flex justify-center relative">
      <v-card width="500" max-width="600" max-height="600" class="card-border">
        ...
        <v-card-text class="relative mt-2 px-0 line-list-container d-flex flex-column">
          <div class="px-4 pb-6">
            <v-form ref="pathForm" v-model="valid" @submit.prevent>
  ...
  export default {
    ...
    computed: {
      ...
      getCurrentTime() {
        ...
        return `${hour > 12 ? '오후' : '오전'} ${hour > 12 ? (hour - 12 < 10 ? `0${hour - 12}` : hour - 12) : hour < 10 ? `0${hour}` : hour}:${minute < 10 ? `0${minute}` : minute}`
      }
    },
    async created() {
    ...
    methods: {
      ...
      isValid() {
        return this.$refs.pathForm.validate()
      },
